...
* How is this project different from the Micro Expander?
  * The Aquarius Micro Expander is similar to the Mini Expander in that it has an AY-3-8910 sound chip and a header for Control Pad input (NOT joystick ports, a pin header). The Micro Expander is different in that it has a 32k memory expansion in it, a USB port, a 3.5mm port, along with a custom ROM that features a PT3 music player, USB ROM reading access, and a customized version of BASIC. The down side is that it blocks the Expansion Port of the Aquarius, so cartridge-based ROM programs (SNAFU, Nightstalker, D&D etc.) can't be used (unless you have the ROM image to load through USB).
